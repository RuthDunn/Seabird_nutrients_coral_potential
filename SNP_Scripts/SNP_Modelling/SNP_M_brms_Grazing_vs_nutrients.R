...
# grazing.model.run <- brm(log1Grazing ~ logNitrogen + (1|Atoll_Island),
#                          data = fish,
#                          iter = 3000, warmup = 1000, chains = 4, cores = 4,
#                          control = list(adapt_delta = 0.99, max_treedepth = 12)) # Resolve divergent transitions 
...
# print(grazing.model.run.scale)
# print(grazing.model.run)

# Check diagnositcs:

# plot(grazing.model.run.scale, ask = FALSE)
# pp_check(grazing.model.run.scale)

# plot(grazing.model.run, ask = FALSE)
# pp_check(grazing.model.run)

# Cool

# save(file="SNP_ModelOutputs/Grazing_Nitrogen_brms.scale.Rdata", list="grazing.model.run.scale")
# save(file="SNP_ModelOutputs/Grazing_Nitrogen_brms.Rdata", list="grazing.model.run")
...
fish.highnn <- as.data.frame(predict(grazing.model.run,
...
fish.lownn <- as.data.frame(predict(grazing.model.run,
...
write.csv(pred.data.highnn, "SNP_Data/Processed/Pred_highnn_Grazing_Nitrogen.csv")
write.csv(pred.data.lownn, "SNP_Data/Processed/Pred_lownn_Grazing_Nitrogen.csv")
...
get_variables(grazing.model.run.scale)

# Plot:
p2 <- ggplot(as_draws_df(grazing.model.run.scale)) +
  stat_halfeye(aes(y = b_scalelogNitrogen,  x = 5), point_interval=median_hdi,
               .width=c(.8,.5),  alpha = .4, fill = "#ed9fc5", fatten_point = 2, slab_alpha = .6) +
...
# Model variable names:
get_variables(grazing.model.run)

# Extract conditional effects:
ce.Erosion.Nitrogen <- conditional_effects(grazing.model.run, prob = .80,
                                           effects = 'logNitrogen', resp = "sqrtErosion",
                                           plot = FALSE)[[1]]

# Extract partialised residuals
pr.Erosion.Nitrogen <- partialize(grazing.model.run,
                                  vars= "logNitrogen", resp = 'sqrtErosion', data = fish)



p1 <- pr.Erosion.Nitrogen %>%
  select(logNitrogen, log1Grazing)%>%
  ggplot(aes(x = logNitrogen, y = log1Grazing)) +
  ...
  ylab(expression("log(x+1) Grazing (% reef year"^-1*")")) +
  theme_bw()+
